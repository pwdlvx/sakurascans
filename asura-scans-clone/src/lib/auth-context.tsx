...
// Store user accounts and passwords separately for security
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedAccounts) {
      try {
        ...
        setAccounts(accountsData.map((account: StoredAccount) => ({
          ...account,
  ...
  useEffect(() => {
    // Save users whenever users change
    localStorage.setItem('sakura-users', JSON.stringify(users));
  }, [users]);

  useEffect(() => {
    // Save accounts whenever accounts change
  ...
  const deleteUser = (userId: string) => {
    // Don't allow deleting the current user or admin
