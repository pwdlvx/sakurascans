...
function PopularCarousel({ onAuthRequired }: { onAuthRequired: () => void }) {
  ...
  const scrollToPosition = useCallback((slideIndex: number) => {
  ...
  const autoScroll = useCallback(() => {
    ...
    setCurrentSlide(prevSlide => {
      const nextSlide = prevSlide + 1 >= popularComics.length ? 0 : prevSlide + 1;
      scrollToPosition(nextSlide);
      return nextSlide;
    });
  }, [isAutoScrolling, popularComics.length, scrollToPosition]);

  // Auto-scroll effect - Fixed infinite loop by removing currentSlide from dependencies
  useEffect(() => {
  ...
  }, [isAutoScrolling, autoScroll, popularComics.length]);
          ...
          {popularComics.map((comic, index) => (
            <Link key={`popular-desktop-${comic.id}-${index}`} href={`/series/${comic.id}`} className="comic-card group flex-shrink-0 w-48">
              ...
              <div className="comic-info">
                <h4 className="comic-title mb-2 line-clamp-2" title={comic.title}>{comic.title}</h4>
        ...
        >
          {popularComics.map((comic, index) => (
            <Link key={`popular-mobile-${comic.id}-${index}`} href={`/series/${comic.id}`} className="comic-card group flex-shrink-0 w-40 snap-start">
              ...
              <div className="p-3">
                <h4 className="font-semibold text-xs mb-1 line-clamp-2 leading-tight" title={comic.title}>{comic.title}</h4>
...
export default function Home() {
  ...
  const getSidebarComics = useCallback(() => {
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {featuredComic.genres.slice(0, 4).map((genre) => (
                            <span key={genre} className="text-xs bg-sakura-primary/20 text-sakura-primary px-2 py-1 rounded">
                              {genre}
                            </span>
                          ))}
                        </div>
                ...
                <div className="relative rounded-lg overflow-hidden sakura-glow-subtle h-full cursor-pointer">
                  ...
                  <div className="relative z-10 p-4 flex flex-col justify-between h-full min-h-[280px]">
                    ...
                    <div className="text-left">
                      <div className="mb-2">
                        <h3 className="text-sm font-bold text-white leading-tight">SAKURA SCANS <span className="sakura-text-primary">TRENDING</span></h3>
